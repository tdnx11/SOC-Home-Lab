...
The GPO enables successful process creation auditing using:
...
Process creation auditing generates **Windows Security Event ID 4688** when a new process is created.
...
This provides additional context during investigations by allowing process creation events to capture command-line arguments.

Wazuh can therefore provide telemetry including:
...
As part of the Windows Process Creation investigation, I executed:

```cmd
net user
```

The command generated Windows Event ID `4688`, which was collected by the Wazuh agent and forwarded to the Wazuh server.
...
This demonstrated how process and command-line telemetry can provide context when investigating account discovery activity.

Although `net user` has legitimate administrative uses, it can also be used for account discovery. In a real investigation, additional context would be reviewed before determining whether the activity was malicious.

➡️ [View the full investigation](investigations/03-Windows-Process-Creation-Investigation.md)
